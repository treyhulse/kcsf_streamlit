...
def main():
    ...
    # Step 1: Load and display the form
    with st.form(key='visualization_form'):
        x_column = st.text_input("Select X-axis column", "Date")
        y_column = st.text_input("Select Y-axis column", "Amount")
        ...
        color_column = st.text_input("Color By (optional)", "")
        preview_button = st.form_submit_button("Preview Visualization")

    # Step 2: After form submission, fetch data and render visualization
    if preview_button:
        ...
        try:
            ...
            if data.empty:
                ...
                if fig:
                    st.plotly_chart(fig)
                    # Step 3: Offer the option to save the visualization
            ...
            logging.error(f"Error fetching data or rendering visualization: {e}")
